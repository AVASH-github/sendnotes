        ...
        @else
            ...
            <div class="grid grid-cols-3 gap-4">
                @foreach ($notes as $note)
                    <x-card wire:key='{{$note->id}}' mt-120>
                        <div class="flex justify-between">
                            <div>
                              @can('update', $note)
    <a href="{{ route('notes.edit', $note->id) }}" wire:navigate class="text-xl font-bold hover:underline hover:text-blue-500">{{ $note->title }}</a>
@else
    <p class="text-xl font-bold text-gray-500">{{ $note->title }}</p>
@endcan
                                <p class="mt-4 text-xs">{{ Str::limit($note->body, 50) }}</p>
                            </div>
                        ...
                        <div class="flex items-end justify-between mt-4 space-x-1">
                            ...
                            <div>
                                <x-button flat icon="eye" href="{{ route('notes.view', $note) }}"></x-button>
